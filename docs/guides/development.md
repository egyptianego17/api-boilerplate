...
For new modules, prefer the project's hand-rolled recipe at [`add-a-module.md`](./add-a-module.md) — it produces files that already match the boilerplate's conventions (custom field decorators, `@Protected`, `AbstractEntity`, exception classes, etc.). The NestJS CLI generators below are useful for quick stubs but produce code you'll need to retrofit.

```bash
# Built-in NestJS CLI schematics
npx nest generate module feature-name
npx nest generate service feature-name
npx nest generate controller feature-name
npx nest generate resource feature-name      # full CRUD scaffold

# Project-aliased generator (uses awesome-nestjs-schematics)
npm run generate service feature-name
npm run generate controller feature-name
```

After running any generator, manually align the output with [`../conventions/coding-standards.md`](../conventions/coding-standards.md) — the generated files do not use this project's custom decorators or base classes.
